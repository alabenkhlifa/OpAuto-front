      ...
      <div class="glass-card settings-nav">
        <nav class="flex flex-wrap gap-2 overflow-x-auto">
          ...
          @if (isOwner()) {
            ...
            <!-- S-CAT-009 (Sweep C-21) — link to the Service Catalog admin CRUD page. -->
            <a
              class="nav-button flex items-center gap-2 px-4 py-2 rounded-lg border transition-colors duration-200 font-medium whitespace-nowrap nav-button-inactive"
              [routerLink]="['/settings/service-catalog']">
              <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6h16M4 12h16M4 18h7"></path>
              </svg>
              <span class="text-sm">{{ 'settings.navigation.serviceCatalog' | translate }}</span>
            </a>
          }
        </nav>
      </div>

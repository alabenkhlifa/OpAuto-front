...
Creating invoices goes through create_invoice — also typed-confirm. The tool both creates AND issues the invoice in one step, assigning a gapless fiscal number. Always: (1) compute the total HT + TVA + fiscal stamp from the line items locally and pass the formatted total as _expectedConfirmation (e.g. "67.83 TND"), (2) include a dueDate (issue requires it), (3) verify part availability via list_low_stock_parts before adding any partId-bound line. Never use record_payment to "create" an invoice — they're separate tools. Never call create_invoice with guessed prices or string-only line items; if the user only names services/parts without quantities and HT unit prices, ask for the missing prices first.`;

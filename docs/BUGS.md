...
- **Where:** `src/app/features/invoicing/components/payment-modal/payment-modal.component.ts` — `onSubmit()` does not guard against re-entry; `src/app/features/invoicing/components/invoice-details.component.ts` — `onPaymentModalSubmit()` calls `addPayment()` once per emit; **and** the modal does not disable the submit button while the request is in flight.
...
- **Surfaced by:** Sweep C-15 staff-seed walk (2026-05-02) — single click on the Submit button via Chrome DevTools MCP `click()` produced 2 POSTs. Likely the `(click)` listener and a default form-submit handler are both firing, OR the click bubbles through a wrapping form element with its own submit.
- **Suggested fix:** (1) flip `isSubmitting()` to `true` before the network call and disable the submit button via `[disabled]="isSubmitting()"`; (2) consider a `(submit)` handler with `event.preventDefault()` if the form-level submit is firing; (3) idempotency-key on the BE side via a request UUID so accidental retries become no-ops at the DB layer.
- **Priority:** P3 — first call always succeeds; user-visible state is correct; this is console-spam + a small server-side log noise issue, not a data corruption risk. Worth tightening before any payment-flow polish sweep.
...
- **Suggested fix:** split read vs write at the route level — `GET /garage-settings` and `GET /garage-settings/fiscal` should be `@Roles(UserRole.OWNER, UserRole.STAFF)` (STAFF needs to know the garage's TVA + RIB to render an invoice correctly), while `PATCH` / `PUT` mutating endpoints stay `@Roles(UserRole.OWNER)`. Mirror the invoicing controller's split. Add a regression spec in `garage-settings.controller.spec.ts` for the STAFF-readable / OWNER-writable matrix.
- **Priority:** P3 — pre-existing condition surfaced by Sweep C-15, NOT introduced by this sweep. Page renders correctly for STAFF; no functional regression. Worth fixing in a follow-up to keep the console clean and to enable any future STAFF-facing fiscal-config UI.

...
| BUG-115 | Help modal sections squashed instead of body scrolling on overflow. Symptom: with one section expanded, the collapsed section headers ("Tools", "Skills", "Specialists") were clipped in half on mobile (~30px tall instead of natural ~52px) and on desktop the body refused to scroll at all when the expanded section's `<ul>` was taller than the modal. Root cause: `.assistant-help-modal__body` is a `display: flex; flex-direction: column; overflow-y: auto` container, but each `<section>` child inherited the default `flex-shrink: 1` — when content overflowed, the browser shrank ALL flex children proportionally instead of triggering the body's `overflow-y: auto`. That collapsed the toggles past their natural height and `overflow: hidden` on the section clipped the title text. Fix: `flex-shrink: 0` on `.assistant-help-modal__section` so children retain their intrinsic height and the body's own scroll kicks in. Also added `-webkit-overflow-scrolling: touch` on the body for smoother iOS momentum scroll. Per UX request, also flipped the default-expanded section from `'tools'` to `null` so all sections start collapsed and the user picks what to explore. Verified end-to-end at 1920×1080, 768×1024, and 390×844: all three toggles render at 52px regardless of which section is expanded; body scrolls (scrollHeight 4800 vs clientHeight 740 on mobile with Tools open); sticky header stays put while items shift. | Assistant | 🟢 |
| BUG-116 (`205d674`) | Mobile sidebar covered by sticky page headers. Symptom: opening the mobile sidebar from `/invoices` (and any feature page with a sticky shell header) showed the page header — "Invoicing" / "Generate and manage invoices" / the Dashboard tab combobox — bleeding on top of the sidebar nav (Cars, Maintenance, Parts & Inventory, Invoicing sub-tree). The global top-bar pills (notifications, language, profile) also stayed visible above the drawer. Root cause: equal `z-index: 30` between `.sidebar` (default) and `.invoicing-shell__header` / `.invoice-form` sticky headers, plus `z-index: 50` on `.app .top-bar` — when DOM siblings tie on z-index, later painted elements win, and the sidebar `<aside>` is rendered before the routed page content so all those page-level stickies stacked on top. The mobile backdrop was at Tailwind `z-20`, lower still. Fix: scoped to the sidebar component only (one place, surgical) — bumped `.sidebar` to `z-index: 60` inside the `@media (max-width: 1023px)` branch, swapped the backdrop's Tailwind class from `z-20` to `z-50`, and lifted `.mobile-menu-btn` from 51 to 70 so the burger ⇆ X icon stays clickable above the open drawer. Desktop z-indexes (30 column / 40 hover-expand) untouched. Modal stack lives at 1000+ so no collision (assistant launcher 1000, panel 1001, help modal 1100). Verified at 390×844, 768×1024, and 1920×1080 — drawer fully covers `/invoices` and `/customers`, X button closes correctly, no console errors, full Karma suite 1062/1062 green. Other shells with the same `z-index: 30` sticky pattern (`invoice-form.component.css:25`) automatically benefit since the fix is on the sidebar side. | Sidebar | 🟢 |

...
- **Suggested fix:** treat collapsed-mode as a rail (~64 px width) instead of an off-screen slide. Concretely: drop the `translateX(-100%)` rule, add a `.sidebar.collapsed { width: 64px; }` rule, hide labels / chevrons / submenus by `[class.hidden]="!shouldShowExpanded()"` (already wired), keep the icon column visible, and on `:hover` (desktop) overlay the expanded panel either via the existing `shouldShowExpanded()` (already returns true on hover) or via a per-parent flyout for children. The expand toggle then becomes "expand on hover / pin on click" — matches every other ERP rail. Likely a 1–2 hour redesign, includes a UI-system review.
- **Priority:** P2 — collapsed rail is rarely used (most owners keep the sidebar pinned), but the current pattern hides the navigation entirely behind a hamburger which is unusual on desktop.

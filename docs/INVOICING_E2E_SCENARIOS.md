...
| S-SB-006 | Sidebar collapsed mode: invoicing icon visible, hover shows children | P2 | ⚠️ (Sweep C-11 — current behaviour is "fully hidden + show-button": `.sidebar.collapsed { transform: translateX(-100%) }` slides the 280 px rail off-screen, leaving only a 44 px floating hamburger; the invoicing `.nav-icon` reports `getBoundingClientRect().x = -53` post-collapse and hover does nothing because there's no on-screen surface. The rail-mode pattern (~64 px icon strip + hover-flyout) is a product redesign — logged as **BUG-107 🔴 P2**, deferred to a UI-system pass) |

...
**Last updated:** 2026-05-02 after Sweep C-3 — quote line types (Section 4: S-QUO-005 / 006 / 007). 3 ✅ flips + 1 inline bug fix (**BUG-104** — quote-form line-type combobox `[selected]` shim). +12 new specs in `quote-form.component.spec.ts`. **Section 4 (Quotes) is now 14 ✅ + 2 🟡 backend + 7 ❌** — remaining ❌ are state-machine edges (reject / re-send / discount-threshold), not line-type behaviour.

...
**Last updated:** 2026-05-01 after Sweep C-4 — quote lifecycle (Section 4: S-QUO-013 / 014 / 015 / 016 / 019 / 020 / 022). 7 ✅ flips + 1 inline FE-mapping fix (**BUG-105 🟢** — `QuoteService.approve` was reading `res.invoiceId` but BE returns `{ quote, invoice }`). +29 new specs across `quote-detail.component.spec.ts`, `quote-form.component.spec.ts`, `quote-list.component.spec.ts` (new file), `quote.service.spec.ts` (new file). New affordances: APPROVED→converted-invoice link block on quote-detail (S-QUO-020); FE-only discount-approver gate on quote-form mirroring invoice-form (S-QUO-022). **Section 4 (Quotes) is now 100 % verified — 21 ✅ + 2 🟡 backend; 0 ❌.** Sections 4 / 5 / 6 / 7 are all green.

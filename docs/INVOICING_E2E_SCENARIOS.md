...
**Last updated:** 2026-05-01 after Sweep C-6 — delivery (Section 10: S-DEL-009 / 010 / 011 / 012). 4 ✅ flips + 1 bonus pre-existing test fixed (`SendInvoiceModalComponent — trims whitespace`). +30 new specs across `send-invoice-modal.component.spec.ts` (+22) and `invoice-details.component.spec.ts` (+8). New affordances: real `tunisianPhoneValidator()`, translated subject/body preview pane, Send-button gate widened to PARTIALLY_PAID/OVERDUE/PAID, `formValidSignal` bridging reactive-form validity → signal-driven `canSubmit`. **Section 10 (Delivery) is now 5 ✅ + 7 🟡 backend; 0 ❌.** Sections 4 / 5 / 6 / 7 / 8 / 9 / 10 are all green.

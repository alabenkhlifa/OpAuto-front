...
### I-001 — Register a `create_invoice` assistant tool ✅ closed (2026-05-04)
**Why:** B-17 had to be skipped because `create_invoice` was never registered — the assistant could record payments but couldn't issue the invoice in the first place. The TYPED_CONFIRM_WRITE infrastructure existed but `record_payment` was its only consumer.
**Shipped:** new `buildCreateInvoiceTool` in `opauto-backend/src/assistant/tools/invoicing-inventory/create-invoice.tool.ts`. TYPED_CONFIRM_WRITE, OWNER-only, gated on the `invoicing` module flag. Handler verifies customer + car ownership against `ctx.garageId`, calls `InvoicingService.create()` for the DRAFT, then `InvoicingService.issue()` to assign a gapless fiscal number via `NumberingService.next()` and lock the invoice (Tunisian compliance). On `issue()` failure (e.g. insufficient stock) the orphan DRAFT is best-effort cleaned up via `InvoicingService.remove()` so the tool is atomic from the user POV. `_expectedConfirmation` carries the formatted total the LLM computed; the orchestrator's TYPED_CONFIRM flow validates the user typed it back before approval.
**Tests:** 11 new specs in `invoicing-inventory-tools.spec.ts` covering schema (TYPED_CONFIRM tier, OWNER role, empty-lines reject, negative qty/price reject, zero unitPrice OK for courtesy lines, minimal payload accepted), cross-tenant guards (foreign customer, foreign car, car-belongs-to-other-customer), happy path (create → issue → return fiscal number + total + dueDate), atomic semantics (issue failure → DRAFT removed; remove failure → original error still surfaces), and per-line tvaRate pass-through.
**Wiring:** registered in `invoicing-inventory-tools.registrar.ts` (tool count 29 → 30); added to `finance-agent` whitelist + finance-agent system prompt updated with the create-invoice playbook (compute total HT+TVA+stamp locally → pass as `_expectedConfirmation` → require dueDate → check stock first for partId-bound lines); frontend presenter entry in `tool-presenters.ts`; en/fr/ar i18n keys added (assistant.help.tools.create_invoice + assistant.tools.create_invoice).
